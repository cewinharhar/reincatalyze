...
if __name__ == "__main__":
    ...
    dici = dict(
        reference_pdb = 'data/raw/test/ortho12_FE_oxo.pdb', 
        reference_ligand_pdbqt = "/home/cewinharhar/GITHUB/reincatalyze/data/raw/test/akgd31_sub9_4_renamed.pdb", 
        reference_ligand_id = 3,
        target_protein_pdb = "data/raw/test/akgd31.pdb", 
        target_ligand_pdbqt= "data/raw/test/akgd31_sub94.pdb", 
        target_ligand_id = 3, 
...
for i in reference_structure["A"].get_residues():
    list(i.get_atoms())
    break
...
pdbqt_to_pdb=f"./Vina-GPU --receptor {receptor} --ligand {ligand4Cmd} --thread {thread}\
                --seed {seed} --center_x {cx} --center_y {cy} --center_z {cz}  \
                --size_x {sx} --size_y {sy} --size_z {sz} \
                --out {ligandOutPath} --num_modes {num_modes} --search_depth {exhaustiveness}"

ps = subprocess.Popen([pdbqt_to_pdb],shell=True, stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
stdout, stderr = ps.communicate(timeout=100)
...
def rename_atoms(pdb_filename, pdb_output):
...
rename_atoms(
    "/home/cewinharhar/GITHUB/reincatalyze/data/raw/test/akgd31_sub9_4.pdb",
    "/home/cewinharhar/GITHUB/reincatalyze/data/raw/test/akgd31_sub9_4_renamed.pdb"
)
